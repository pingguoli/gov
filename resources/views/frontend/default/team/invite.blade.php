                                    ...
                                    @if(!empty($list))
                                        @foreach($list as $entry)
                                            @if($entry->status == 0)
                                                <div class="card">
                                                    <div class="card-body">
                                                        <h4 class="card-title">{{ $entry->team->name }}</h4>
                                                        <p class="card-text">
                                                            <a href="{{ route('team.confirminvite', ['code' => $code, 'id' => $entry->id]) }}" data-val="1" class="btn btn-sm btn-outline-info confirm-invite">同意</a>
                                                            <a href="{{ route('team.confirminvite', ['code' => $code, 'id' => $entry->id]) }}" data-val="0" class="btn btn-sm btn-outline-danger confirm-invite">拒绝</a>
                                                        </p>
                                                    </div>
                                                </div>
                                            @endif
