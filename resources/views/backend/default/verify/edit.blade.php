    ...
    <form method="post" enctype="multipart/form-data">
        ...
        <div class="box box-primary">
            ...
            <div class="box-body pad-10">
                    ...
                    <div class="form-group @if($errors->has('type')) has-error @endif">
                        ...
                        <select class="form-control" id="type" name="type">
                            ...
                            @foreach(\App\Model\Verify::$types as $key => $val)
                                <option value="{{ $key }}" @if(old('type') == $key || old('type') === null && $verify->type == $key) selected @endif >{{ $val }}</option>
                            @endforeach
                        </select>
                        @if($errors->has('type'))
                ...
                <div id="project" class="@if(old('type') !== null && !in_array(old('type'), \App\Model\Verify::$projectSome) || old('type') === null && !in_array($verify->type, \App\Model\Verify::$projectSome)) hide @endif">
                    <div class="col-lg-4">
                            ...
                            <select class="form-control" id="project_id" name="project_id">
                                ...
                                @if(!empty($projects))
                                    @foreach($projects as $item)
                                        <option value="{{ $item->id }}" @if(old('project_id') == $item->id || old('project_id') === null && $verify->project_id == $item->id) selected @endif >{{ $item->name }}</option>
                                    @endforeach
                                @endif
                            </select>
                        ...
                        <select class="form-control" id="admin_id" name="admin_id">
                            ...
                            @if(!empty($managers))
                                @foreach($managers as $item)
                                    <option value="{{ $item->id }}" @if(old('admin_id') == $item->id || old('admin_id') === null && $verify->admin_id == $item->id) selected @endif >{{ $item->nickname ?: $item->username }}</option>
                                @endforeach
                            @endif
                        </select>
