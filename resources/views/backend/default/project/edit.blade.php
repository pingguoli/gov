            ...
            <div class="box-body pad-10">
                        ...
                        <select class="form-control" id="type_id" name="type_id">
                            ...
                            @if(!empty($projectTypes))
                                @foreach($projectTypes as $item)
                                    <option value="{{ $item->id }}" @if(old('type_id') == $item->id || old('type_id') === null && $project->type_id == $item->id) selected @endif >{{ $item->name }}</option>
                                @endforeach
                            @endif
                        </select>
                ...
                <div class="col-lg-4">
                    <div class="form-group @if($errors->has('name')) has-error @endif">
                        ...
                        <input class="form-control " name="name" type="text" placeholder="{{ __('Name') }}" value="{{ old('name') ?: $project->name }}">
                        @if($errors->has('name'))
